...
<body>
		...
		<div align="left" style="width: 100%; padding-bottom: 10px;">
			<span style="font-size: 30px; font-weight: bold;">&quot${
				query }&quot 연관 영화</span>
		</div>
		<div style="display: flex; position: relative">

			<button id="searchMoviesLeftBtn" class="leftBtn"
				onclick="searchMoviesLeftBtn();">◀</button>

			<c:forEach var="list" items="${ searchList1 }">
				<%
					if (cnt < 5) {
				%>
				<a href="/movieInfo?movieId=${ list.movieId }" class="searchMovies"
					style="all: unset; cursor: pointer; display:;">
					<div style="padding-right: 10px;">
						...
						<c:if test="${ list.posterUrl eq 'nan' }">
							<div
								style="width:${imgWidth}px; height:${ imgHeight }px; border:1px solid rgba(0, 0, 0, 0.1); display:flex; justify-content:center; align-items:center;">이미지가
								없습니다.</div>
						</c:if>
						<div align="left">
							<c:if test="${ list.movieNm.length() > 15 }">
								<span style="font-size: 18px; font-weight: bold">${ list.movieNm.substring(0, 15) }...</span>
								<br>
							</c:if>
							<c:if test="${ list.movieNm.length() <= 15 }">
								<span style="font-size: 18px; font-weight: bold">${ list.movieNm }</span>
								<br>
							</c:if>
					...
					<div style="padding-right: 10px;">
						...
						<div align="left">
							<c:if test="${ list.movieNm.length() > 18 }">
								<span style="font-size: 18px; font-weight: bold">${ list.movieNm.substring(0, 18) }...</span>
								<br>
							</c:if>
							<c:if test="${ list.movieNm.length() <= 18 }">
								<span style="font-size: 18px; font-weight: bold">${ list.movieNm }</span>
			...
			<%
				if (cnt > 5) {
			%>
			<button id="searchMoviesRightBtn" class="rightBtn" type="button"
				onclick="searchMoviesRightBtn('<%=cnt%>');">▶</button>
			<%
				}
			%>
		</div>
		...
		<c:if test="${ memberCommend.size() > 0 }">
			<div align="left" style="width: 100%; padding-bottom: 10px;">
				<span style="font-size: 30px; font-weight: bold;">회원님만을 위한
					추천영화</span>
			</div>
			<div style="display: flex; position: relative">

				<button id="memberCommendLeftBtn" class="leftBtn"
					onclick="memberCommendLeftBtn();">◀</button>
				...
				<%
					if (cnt > 5) {
				%>
				<button id="memberCommendRightBtn" class="rightBtn" type="button"
					onclick="memberCommendRightBtn('<%=cnt%>');">▶</button>
				<%
					}
				%>
			</div>
		...
		<c:forEach var="movieInfo" items="${ searchList1 }">
			<a style="all: unset; cursor: pointer;"
				href="movieInfo?movieId=${ movieInfo.movieId }">
				<div style="display: flex;">
					...
					<div align="left" style="width: 50%;">
						<br>
						<br>
						<br> <span style="font-size: 40px; font-weight: bold;">${ movieInfo.movieNm }</span>
						<br>
						<br>
			...
			<c:if test="${ movieInfo.movieActorList.size() > 0 }">
				...
				<div style="display: flex;">
					<c:forEach var="peopleInfo" items="${ movieInfo.movieActorList }">
						<c:if
							test="${ not empty peopleInfo.cast && peopleInfo.peopleId ne '0' }">
							<a href="peopleInfo?peopleId=${ peopleInfo.peopleId }"
								style="all: unset; cursor: pointer; display: flex; width: 25%;">
								<div>
						...
						<c:if
							test="${ not empty peopleInfo.cast && peopleInfo.peopleId eq '0' }">
