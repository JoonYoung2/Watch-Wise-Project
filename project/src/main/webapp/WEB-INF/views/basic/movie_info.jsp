...
<input type="hidden" id="gradeCnt" value="${ gradeInfo.gradeCnt }">
   ...
   <div align="center" style="width:100%;">
      <div align="center" style="width:80%; display:flex;">
		...
		<!-- MemberInfoController에 있음. -->	<img src="/resources/img/saved.png" onclick="location.href='/deleteMovieFromWishList?movieId=${ movieInfo.movieId }'" style="cursor:pointer; width:40px; margin-bottom:10px;"><br>
							    			저장됨
						    				</c:when>
						    				<c:otherwise>
		<!-- MemberInfoController에 있음. -->	<img src="/resources/img/plus.png" onclick="location.href='/addMovieIntoWishList?movieId=${ movieInfo.movieId }'" style="cursor:pointer; width:40px; margin-bottom:10px;"><br>
							    			보고싶어요
							    			</c:otherwise>
							    		</c:choose>
							    		</div>
							    		...
							    		<c:choose>
											<c:when test="${sessionScope.isBlack eq 1 }"><!-- 블랙리스트일 경우 -->
										    	<div style="width:30%; font-size:13px; margin-right:5px;">
													<img src="/resources/img/thinPencil.png" onclick="openBlackModal()" style="cursor:pointer;width:40px; margin-bottom:10px;"><br>
													코멘트
												</div>
											</c:when>
											...
											<c:otherwise><!-- 블랙리스트가 아닐 경우 -->
									    		<c:choose>
										    		...
										    		<div style="width:30%; font-size:13px; margin-right:5px;">
										    			<img src="/resources/img/thinPencil.png" onclick="openCommentWritingModal('${ movieInfo.movieNm }', '${ movieInfo.movieId }')" style="cursor:pointer;width:40px; margin-bottom:10px;"><br>
										    			코멘트
										    		</div>
										    		</c:when>
										    		<c:otherwise> <!-- 코멘트 이미 작성된 경우 -->
										    		<div style="width:30%; font-size:13px; margin-right:5px;">
										    			<img src="/resources/img/thinPencil.png" onclick="openModifyModal('${ movieInfo.movieNm }' , '${movieInfo.movieId }', '${ifWroteComment.reviewComment}');" style=" cursor:pointer;width:40px; margin-bottom:10px;"><br>
										    			코멘트
										    		</div>
										    		</c:otherwise>
										    	</c:choose>
								    		...
								    		<div>
												<c:if test="${not empty sessionScope.userEmail }">
													<div align="center" id="review" style="">
														<c:if test="${ ifWroteComment.reviewScore ne 0.0 && not empty ifWroteComment.reviewScore }">
															<select id="select-box" name="rating" style="all:unset; font-size:25px; text-align:center; align-items:center; width:150px; height: 52px; border:1px solid rgba(0, 0, 0, 0.1); color:orange; width:100%;" onchange="rating(this.value,'${movieInfo.movieId }');">
														...
														<c:if test="${ ifWroteComment.reviewScore eq 0.0 || empty ifWroteComment.reviewScore }">
															<select id="select-box" name="rating" style="all:unset; font-size:25px; text-align:center; align-items:center; width:150px; height: 52px; border:1px solid rgba(0, 0, 0, 0.1); color:red; width:80%;" onchange="rating(this.value,'${movieInfo.movieId }');">
                            ...
                            <div style="text-align:center; margin:10px;background-color:#F2F2F2; width:350px; height: 300px; border-radius:5px; overflow: hidden;">
                                <div style="margin:10px; width:304px; height:280px;">
                                	...
                                	<div class="하트" style=" display:flex; margin-left:10px; height:10%; padding-top:8px;border-top: 1px solid #E5E5E5;">
                                		...
                                		<div class="" style="text-align:right; margin-right:5px;width:50%;">
	                                		<c:choose>
